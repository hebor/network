...
在R2的ospf进程中补充宣告公网口网段`23.0.0.0 0.0.0.255`后，ospf建立邻接关系的过程会反复的Down和Full。这个问题是由于 *路由表查找* 和 *GRE的加封装* 引起的
...
R2将公网地址宣告到隧道口后，GRE加封装时就会出现问题。`192.168.10.0/24`通过GRE加封装公网IP报头`23.0.0.0/24`后，仍需要再次查找R1的路由表进行出站，但此时由于R2使用ospf宣告了公网接口，那么R1的路由表中会新增一条通过ospf学习到的去往`23.0.0.0/24`的路由，且其出接口为Tunnel隧道口，那么在GRE加封装后通过查找R1路由表进行出站时，由于ospf优先级高于缺省路由，因此GRE封装后的数据会通过ospf路由进行出站，而ospf的路由出接口又指向Tunnel隧道口，因此又会重新触发GRE加封装，如此反复处于路由递归的过程
