...
配置BFD链路与接口状态联动，当SW3与SW4的互联链路故障时，BFD链路检测到故障后，交换机三层接口状态也自动切换为Down状态，查看端口信息时会看到物理端口的Protocol状态为up(b)，b代表着BFD Down
...
*当SW3与SW4的互联链路故障时*，BFD会更快检测到链路故障并反馈到VRRP，由于没有其他机器与SW1竞争Master角色，因此分别查看SW1和SW2的VRRP状态，两者哦都认为自己是Master，但实际上现在应该是SW2的VRRP优先级更高，因为BFD检测到链路故障后会将SW1的VRRP优先级-110，因此现在SW1的优先级应该是90；如果BFD同时与接口状态和VRRP互联，则当SW3与SW4的互联链路故障时，不仅BFD的状态会变更为DOWN，VRRP的状态也会变更为Initialize