...
AS_Path，BGP更新内的重要属性，代表该路由所经过的AS号，如果接收路由器发现AS_Path内存在AS号与本地AS号一致，则丢弃该路由信息。在查看BGP路由表时可能会看到AS_Path内会存在多个相同的AS号，这是因为AS_Path的长度会作为一个选路规则

**BGP路由聚合**

BGP静态聚合：通过配置静态路由汇总明细路由，再用network进行BGP路由宣告