...
Comware操作系统的line命令和user-interface命令都是用于用户线视图配置
...
[R1-line-vty0-63]protocol inbound ssh	#指定用户登录时使用ssh协议
[R1-line-vty0-63]local-user hebor class manage
[R1-luser-manage-hebor]service-type telnet ssh		#增加支持ssh协议
[R1-luser-manage-hebor]password simple Admin@huawei.com
[R1-luser-manage-hebor]authorization-attribute user-role network-admin
...
public-key local destroy rsa	#销毁RSA密钥
```
...
<H3C>save force		#强制保存，配置文件命名为startup.cfg
...
<H3C>terminal debugging	#打开调试信息的屏幕输出楷开关，设置当前终端可以显示调试日志
