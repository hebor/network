...
# H3C设备配置DNS代理
dns proxy enable
display ip host    #查看静态域名解析表
ip host hostname ip-address    #配置静态域名解析表中的主机名与对应地址
dns server ip-address    #配置指定域名服务器的地址，交换机收到内网的域名解析请求后，会向此DNS服务器发起解析请求
display dns server    #查看DNS服务器的地址信息
dns domain domain-name    #配置域名后缀
display dns domain    #查看域名后缀信息
```
