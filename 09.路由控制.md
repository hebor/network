...
#RIP引入路由的种子度量值默认是0，AR3在引入OSPF路由并转发给AR2时，RIP度量值加1，也就是说AR2到AR5、AR6的度量值是1跳，同理AR1到AR5、AR6的度量值是2跳
<AR1>dis ip routing-table
<AR2>dis ip routing-table
#OSPF引入路由的种子度量值默认是1，由于OSPF引入外部路由默认类型是Type2，所以AR5、AR6到RIP路由器的开销值都是1
<AR5>dis ip routing-table pro ospf
<AR5>dis ospf routing

#通过引入路由配置和策略路由配置修改种子度量值和OSPF路由类型
# 1.条件匹配工具
[AR3]acl 2000
...
# 2.策略工具，先测试OSPF引入进RIP的开销
[AR3]route-policy rip permit node 10	
[AR3-route-policy]if-match acl 2001
[AR3-route-policy]apply cost 5
[AR3-route-policy]route-policy rip permit node 20	#定义一条空路由策略，如果不定义此策略，路由策略默认会拒绝所有路由
# 3.调用工具，为所有引入的路由设置开销值，并调用路由策略。路由策略会更优先生效，命中路由策略条件的路由，会修改为路由策略中定义的开销值
[AR3-route-policy]rip 1
[AR3-rip-1]import-route ospf cost 10 route-policy rip	
#在AR1上查看引入的OSPF路由的开销值，13.0.0.0开销为7，36.0.0.0开销为12
<AR1>dis ip routing-table
```