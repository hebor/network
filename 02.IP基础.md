...
华为交换机每次保存设备配置默认会压缩保存到vrpcfg.zip文件，设备配置文件分2种，一种是保存在内存中的`Current-Configuration File`、另一种是保存在Flash或SD卡中的`Saved-Configuration File`，默认所有修改的配置都保存在内存中，如果不手动执行save保存配置命令，那么交换机重启后配置会丢失。未执行save之前，查看`Saved-Configuration File`会提示没有正确的配置文件
